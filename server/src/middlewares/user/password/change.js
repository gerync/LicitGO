...
export default function changePasswordMiddleware(req, res, next) {
    ...
    if (!currentPassword || !newPassword || !newPasswordConfirm) {
        return res.status(400).json({ error: lang === 'HU' ? 'Hiányzó adatok.' : 'Missing data.' });
    }
    ...
    if (newPassword !== newPasswordConfirm) {
        return res.status(400).json({ error: lang === 'HU' ? 'Az új jelszavak nem egyeznek.' : 'New passwords do not match.' });
    }
