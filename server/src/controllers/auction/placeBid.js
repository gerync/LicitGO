...
export default async function placeBidController(req, res) {
    ...
    try {
        ...
        if (auctionRows.length === 0) {
            throw [ lang === 'HU' ? 'Az aukció nem található.' : 'Auction not found.', 404 ];
        }
        const auction = auctionRows[0];
        // #endregion
        // Debug: log owner token vs requester token to diagnose false positives
        console.log('DEBUG placeBid owner token:', auction.ownertoken, 'request usertoken:', usertoken, 'types:', typeof auction.ownertoken, typeof usertoken);
        ...
        if (usertoken === auction.ownertoken) {
            throw [ lang === 'HU' ? 'Nem licitálhatsz a saját aukcióidra.' : 'You cannot bid on your own auctions.', 400 ];
        }
        const currentMaxBid = bidRows[0].maxBid || 0;
        // #endregion
        // #region Licit ellenőrzése
        if (bidAmountUSD <= currentMaxBid) {
            throw [ lang === 'HU' ? 'A licit összegének nagyobbnak kell lennie a jelenlegi legmagasabb licitnél.' : 'Bid amount must be higher than the current highest bid.', 400 ];
        }
        if (bidAmountUSD < auction.startingpriceUSD) {
            throw [ lang === 'HU' ? 'A licit összegének el kell érnie a kezdő licit összegét.' : 'Bid amount must meet the starting bid amount.', 400 ];
        }
        const now = new Date();
        const auctionStart = new Date(auction.starttime);
        const auctionEnd = new Date(auction.endtime);
        if (now < auctionStart) {
            throw [ lang === 'HU' ? 'Az aukció még nem kezdődött el.' : 'The auction has not started yet.', 400 ];
        }
        if (now > auctionEnd) {
            throw [ lang === 'HU' ? 'Az aukció már lejárt.' : 'The auction has already ended.', 400 ];
        }
        // #endregion
