...
async function PostBidHU(req, res) {
  ...
  if (typeof req.body !== 'object' || req.body === null || Object.keys(req.body).length !== 3) {
    conn.release();
    return res.status(400).json({ error: 'Érvénytelen kérés törzse' });
  }
  ...
  else if (isNaN(auction_id) || auction_id <= 0 || typeof auction_id !== 'number' || auction_id % 1 !== 0) {
    conn.release();
    return res.status(400).json({ error: 'Érvénytelen aukció azonosító' });
  }
  else if (isNaN(bidder_id) || bidder_id <= 0 || typeof bidder_id !== 'number' || bidder_id % 1 !== 0) {
    conn.release();
    return res.status(400).json({ error: 'Érvénytelen licitáló azonosító' });
  }
  else if (isNaN(bid_amount) || bid_amount <= 0 || typeof bid_amount !== 'number') {
    conn.release();
    return res.status(400).json({ error: 'Érvénytelen licit összeg' });
  }
  else {
    if (!(await app.db.query('SELECT id FROM auctions WHERE id = ?', [auction_id]).then(([rows]) => rows.length > 0))) {
      conn.release();
      return res.status(404).json({ error: 'Aukció nem található' });
    }
    else if (!(await app.db.query('SELECT id FROM users WHERE id = ?', [bidder_id]).then(([rows]) => rows.length > 0))) {
      conn.release();
      return res.status(404).json({ error: 'Licitáló felhasználó nem található' });
    }
    const [highestBidRows] = await app.db.query('SELECT MAX(bid_amount) AS highest_bid FROM bids WHERE auction_id = ?', [auction_id]);
    const highestBid = highestBidRows[0].highest_bid || 0;
    if (bid_amount <= highestBid) {
      conn.release();
      return res.status(400).json({ error: 'A licit összegnek nagyobbnak kell lennie a jelenlegi legmagasabb licitnél' });
    }
    try {
      const [result] = await app.db.query(`INSERT INTO bids (auction_id, bidder_id, bid_amount) VALUES (?, ?, ?)`,
